...
def test_rejects_hidden_controller_pointer_decoy() -> None:
    ...
    text = _pkg01_active().replace(
        "### Current control pointer", "### Current control pointer\n\n" + decoy
    )
    prefix, marker, suffix = text.rpartition("**Control package:** `PKG-01`")
    assert marker
    text = prefix + "**Control package:** `PKG-09`" + suffix
    errors = _errors(text)
...
def test_later_unlocked_work_stays_queued_behind_active_frontier() -> None:
    text = _pkg01_active().replace(
        "| 10 | **MNY-08A — current fraud assessments** | PKG-02 | TODO |",
        "| 10 | **MNY-08A — current fraud assessments** | PKG-02 | DONE |",
    )
