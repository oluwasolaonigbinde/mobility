...
3. `distance_m`
   ...
   - MVP allocation rule:
     - For each trip, allocate the tripâ€™s stored distance across cells in proportion to that tripâ€™s ping count in each cell within the requested bbox/date window.
...
4. `estimated_impressions`
   ...
   - MVP allocation rule:
     - For each trip, allocate the tripâ€™s stored estimated impressions across cells in proportion to that tripâ€™s ping count in each cell within the requested bbox/date window.
...
Advertiser cannot read heatmap for another organizationâ€™s campaign.
...
Advertiser cannot read another organizationâ€™s campaign heatmap.
