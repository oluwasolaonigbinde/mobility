...
Safe to commit: Yes. Commit Slice 9 before starting Slice 10.

The parent-delete cascade risk does not block Slice 9 because no destructive parent-delete or settlement/accounting workflow exists yet. Track it as a hardening item before future destructive deletes, settlement, or accounting-grade workflows. The nested-transaction IntegrityError recovery for payout/ledger idempotency is acceptable for this slice because it is backed by database uniqueness constraints and deterministic duplicate-call tests.

Basis: Slice 9 added exactly the approved campaign_payout_rules, payout_calculations, and earnings_ledger_entries tables; implemented the required payout rule, payout calculation, driver earnings, and advertiser cost-summary endpoints; enforced admin/driver/advertiser access boundaries; implemented formula-versioned payout_v1; and preserved out-of-scope boundaries around settlement, withdrawals, billing, heatmaps, and full reporting. 
...
Pasted text
 ...
 The reported checks pass on host/PostGIS and Docker Python 3.12, with Alembic at 0010_payouts_and_earnings (head). 

Pasted text

Recommended commit message:

feat: add payout calculations and earnings ledger

Full Slice 10 implementation prompt:
...
Dashboard summary excludes another organization’s campaigns and metrics.
...
Advertiser cannot read summary for another organization’s campaign.
...
Advertiser cannot list trip summaries for another organization’s campaign.
