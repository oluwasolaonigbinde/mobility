...
Safe to commit: Yes. Commit Slice 12 before starting Slice 13.

Codex question resolved: requiring ALLOW_DEMO_SEED=true for local/dev while allowing test environment to run seed without that override is acceptable. The packet reports production-like environments are refused, demo seed does not run at app startup or through Docker/API automation, and credentials are documented as local-only, so this is a safe and practical testability compromise. 

Pasted text

Basis: Slice 12 added the approved local/demo seed command and docs hardening only, introduced no new migration or tables, preserved Alembic head 0010_payouts_and_earnings, implemented idempotent demo graph creation, created useful advertiser/driver demo data across campaigns, zones, trips, analytics, impressions, payouts, ledger, reports, and heatmaps, and passed host/PostGIS/Docker pytest, ruff, Alembic, seed idempotency, and Python 3.12 Docker verification. 
...
feat: add demo seed data and API docs hardening

Full Slice 13 implementation prompt:
