...
Safe to commit: Yes. Commit Slice 6 before starting Slice 7.

Basis: Slice 6 added exactly the approved trip_sessions, location_ping_batches, and location_pings tables; stores pings as PostGIS geometry(Point,4326); implements driver-only trip lifecycle and durable ping-batch idempotency; passes host/PostGIS/Docker pytest, ruff, and Alembic checks; verifies Python 3.12 in Docker; and confirms no Slice 7+ analytics/fraud/impression/payout/reporting/heatmap scope was implemented. 

Pasted text

Recommended commit message:

feat: add trip tracking and GPS ping ingestion

Full Slice 7 implementation prompt:
