...
**Version 1.43 — 2026-08-24. Canonical source of truth: current state AND target state.**
...
| **RM15** | F09/F10/F11 | PARTIAL — W3-00A build controls delivered; legal approval, W3-00B/C remain | W3-00A now supplies a draft build-only DPIA/ROPA, controller/processor role allocation, purpose/basis/retention/recipient matrix, subprocessor/region and breach registers, withdrawal procedure and synthetic tabletop. Every named legal owner, approved basis/notice/retention decision, provider and region remains MISSING and live use remains false. Remaining gaps: **`k` counts distinct vehicles, which is not a valid anonymity claim**; the existing advertiser heatmap lacks the complete floor; retention/DSR execution across all stores/processors is incomplete. | Before real GPS, qualified counsel must approve the operating artefacts and named owner/bases/notices. W3-00B must execute the per-class retention and manual DSR contract across DB, objects, devices, logs, backups and processors. Before any advertiser heatmap, W3-00C must put one central disclosure service on every query — coarse fixed cells/buckets, minimum vehicles **and** trips **and** days, contributor caps, complementary suppression, restricted filters, atomic query history and differencing tests; thresholds come from real pilot density and outputs remain personal until approved re-identification evidence says otherwise. | §22.2/§24, **before real GPS / before heatmap** |
