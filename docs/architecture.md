...
```
        ...
        ┌───────────────▼───────────────┐
        │  FastAPI  (app/)              │  /api/v1 — 279 operations
   ...
   │ 123 mapped tables,   │   │ counters (F7) + arq queue  │
...
Current OpenAPI: **280 operations across 251 paths**; **279 operations under `/api/v1`** plus root `/health`.

| Prefix | Operations | Paths |
|--------|-----------:|------:|
| `/api/v1/admin/*` | 154 | 140 |
| `/api/v1/advertiser/*` | 60 | 47 |
| `/api/v1/auth/*` | 14 | 14 |
| `/api/v1/driver/*` | 41 | 39 |
| `/api/v1/health*` | 3 | 3 |
| `/api/v1/me` | 1 | 1 |
...
├── models/            # SQLAlchemy models — 123 mapped tables (see §7)
...
- **Administrator grants (AUT-006, D27):** creating an administrator (including
  ...
  in the same transaction, invalidating all earlier bearer, refresh and password
...
### 7.1 Entities **[BUILT]** — 123 mapped tables
...
|---------|---------|-----------|-----------|----------------|------------|
| Public Terrax/Cardvert front door | §27 | `frontend/src/app/page.tsx`, `frontend/src/components/marketing/`, namespaced landing styles/assets | public marketing copy, `/apply`, `/login` and official contact links | auth/role authority; public advertiser signup; product-theme tokens; live-provider claims | [BUILT] provider-neutral; public driver applications still obey runtime enablement and live-use gates |
