...
_LEGACY_DATABASE_COUNTS = {
    ...
    "payout_ledger": (
        ...
        "ON e.id = l.ledger_entry_id WHERE e.driver_user_id = :subject_user_id) + "
        "(SELECT count(*) FROM payout_submission_intents i "
        "JOIN payout_batch_lines l ON l.id = i.payout_batch_line_id "
        "JOIN earnings_ledger_entries e ON e.id = l.ledger_entry_id "
        "WHERE e.driver_user_id = :subject_user_id) + "
        "(SELECT count(*) FROM payout_submission_attempts a "
        "JOIN payout_submission_intents i ON i.id = a.intent_id "
        "JOIN payout_batch_lines l ON l.id = i.payout_batch_line_id "
        "JOIN earnings_ledger_entries e ON e.id = l.ledger_entry_id "
        "WHERE e.driver_user_id = :subject_user_id) + "
        "(SELECT count(*) FROM payout_submission_observations o "
        "JOIN payout_submission_intents i ON i.id = o.intent_id "
        "JOIN payout_batch_lines l ON l.id = i.payout_batch_line_id "
        "JOIN earnings_ledger_entries e ON e.id = l.ledger_entry_id "
        "WHERE e.driver_user_id = :subject_user_id) + "
        "(SELECT count(*) FROM payout_recovery_incidents r "
        "JOIN earnings_ledger_entries e ON e.id = r.ledger_entry_id "
        "WHERE e.driver_user_id = :subject_user_id) + "
