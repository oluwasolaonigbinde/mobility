...
export type CapabilitySnapshot = {
  ...
  // Runtime truth from the server trip state: true only while this client
  // owns an already-started, not-yet-ended trip. The probe page always
  // passes false — offline/unavailable-session capture continuation is an
  // active-trip semantic and must never be authorized from probe evidence.
  activeTrip: boolean;
};
...
export function assessPilotPwa(s: CapabilitySnapshot): CapabilityAssessment {
  ...
  // Probe "pass" for wakeLock/webLocks is capability evidence only — the
  // probe sentinel is released before assessment. A runtime consumer must
  // feed live held-state into these fields (wake-lock release event ->
  // "released", lost writer lock -> "contended") and set activeTrip from the
  // server trip state; capability evidence alone never proves a lock is
  // currently held or a trip is running.
  const ownsWriter = s.webLocks === "pass";
  const sessionUsableForCapture =
    s.session === "valid" ||
    (s.activeTrip && (s.session === "unavailable" || s.session === "offline"));
  const capture =
    installed &&
    s.visibility === "visible" &&
    s.location === "granted" &&
